Significado de cada uma das colunas do dataset:

| Nº  | Coluna         | Descrição                                                           |
| --- | -------------- | ------------------------------------------------------------------- |
| 1   | MSSubClass:    | identifica o tipo de imóvel envolvido na venda.                     |
| 2   | MSZoning:      | identifica a classificação geral de zoneamento da área de venda.    |
| 3   | LotFrontage:   | metros lineares de rua conectados à propriedade                     |
| 4   | LotArea:       | tamanho da propriedade em pés quadrados                             |
| 5   | Street:        | Tipo de acesso rodoviário á propriedade                             |
| 6   | Alley:         | tipo de acesso ao imóvel pelo beco                                  |
| 7   | LotShape:      | formato do terreno do imóvel                                        |
| 8   | LandContour:   | planicidade da propriedade                                          |
| 9   | Utilities:     | tipos de serviços públicos disponíveis                              |
| 10  | LotConfig:     | configuração do lote (se é de esquina, sem saída, ...)              |
| 11  | LandSlope:     | declive do terrenos                                                 |
| 12  | Neighborhood:  | localizações físicas dentro dos limites da cidade de Ames           |
| 13  | Condition1:    | proximidade de determinadas condições                               |
| 14  | Condition2:    | proximidade de determinadas condições se houver mais de uma         |
| 15  | BldgType:      | tipo de habitação                                                   |
| 16  | HouseStyle:    | estilo da moradia                                                   |
| 17  | OverallQual:   | avaliação do material e acabamento geral da casa                    |
| 18  | OverallCond:   | avaliação do estado geral da casa                                   |
| 19  | YearBuilt:     | ano de construção original da casa                                  |
| 20  | YearRemodAdd:  | ano de recuperação ou adição de cômodos na casa                     |
| 21  | RoofStyle:     | tipo do telhado                                                     |
| 22  | RoofMatel:     | material do telhado                                                 |
| 23  | Exterior1st:   | Cobertura externa da casa                                           |
| 24  | Exterior2nd:   | cobertura externa da casa se houver mais um adicional               |
| 25  | MasVnrType:   | tipo de revestimento de alvenaria                                   |
| 26  | MasVnrArea:    | área de revestimento de alvenaria em pés quadrados                  |
| 27  | ExterQual:     | avalia a qualidadee do material no exterior                         |
| 28  | ExterCond:     | avalia o estado atual do material no exterior                       |
| 29  | Foundation:    | tipo de fundação                                                    |
| 30  | BsmtQual:      | avalia a altura do porão                                            |
| 31  | BsmtCond:      | avalia a condição do porão                                          |
| 32  | BsmtExposure:  | refere-se a paredes com saída para o exterior ou ao nível do jardim |
| 33  | BsmtFinType1:  | classificação da área acabada do porão                              |
| 34  | BsmtFinSF1:    | tamanho em pés quadrados do tipo 1                                  |
| 35  | BsmtFinType2:  | classificação da área acabada do porão (se houver vários tipos)     |
| 36  | BsmtFinSF2:    | área construída do tipo 2                                           |
| 37  | BsmtUnfSF:     | área em pés quadrados inacabada do porão                            |
| 38  | TotalBsmtSF:   | área total do porão em pés quadrados                                |
| 39  | Heating:       | tipo de aquecimento                                                 |
| 40  | HeatingQC:     | condição da qualidade do sistema de aquecimento                     |
| 41  | CentralAir:    | possui ar condicionado central?                                     |
| 42  | Electrical:    | sistema elétrico                                                    |
| 43  | 1stFlrSF:      | metragem em pés quadrados do primeiro piso                          |
| 44  | 2ndFlrSF:      | metral em pés quadrados do segundo piso                             |
| 45  | LowQualFinSF:  | baixa qualidade da área construída (todos os andares)               |
| 46  | GrLivArea:     | área habitável acima do nível do solo (em pés quadrados)            |
| 47  | BsmtFullBath:  | banheiros completos no porão                                        |
| 48  | BsmtHalfBath:  | lavabos no porão                                                    |
| 49  | FullBath:      | banheiros completos acima do nível do solo                          |
| 50  | HalfBath :     | lavabos acima do nível do solo                                      |
| 51  | BedroomAbvGr:       | quartos acima do nível do solo (não inclui quartos no porão)        |
| 52  | Kitchen:       | cozinhas acima do nível do solo (não inclui cozinhas no porão)      |
| 53  | KitchenQual:   | qualidade da cozinhas                                               |
| 54  | TotRmsAbsGrd:  | total de cômodos acima do nível do solo (não inclui banheiro)       |
| 55  | Functional:    | funcionalidade da casa                                              |
| 56  | Fireplaces:    | lareiras                                                            |
| 57  | FireplaceQu:   | qualidade das lareiras                                              |
| 58  | GarageType:    | localização da garagem                                              |
| 59  | GarageYrBlt:   | ano em que a garagem foi construída                                 |
| 60  | GarageFinish:  | interior da garagem finalizado                                      |
| 61  | GarageCars:    | quantidade de carros que cabem na garagem                           |
| 62  | GarageArea:    | tamanho da garagem em pés quadradso                                 |
| 63  | GarageQual:    | qualidade da garagem                                                |
| 64  | GarageCond:    | condição da garagem                                                 |
| 65  | PavedDrive:    | entrada de veículos pavimentada                                     |
| 66  | WoodDeckSF:    | área do deck de madeira em pés quadrados                            |
| 67  | OpenPorchSF:   | varanda aberta (em pés quadrados)                                   |
| 68  | EncolsedPorch: | varanda fechada (em pés quadrados)                                  |
| 69  | 3SsnPorch:     | varanda de três estações (em pés quadrados)                         |
| 70  | ScreenPorch:   | varanda tela (em pés quadrados)                                     |
| 71  | PoolArea:      | área da piscina (em pés quadrados)                                  |
| 72  | PoolQC:        | qualidade da piscina                                                |
| 73  | Fence:         | qualidade da cerca                                                  |
| 74  | MiscFeature:   | características diversas não inseridas nas outras categorias        |
| 75  | MiscVal:       | valor das características diversas                                  |
| 76  | MoSold:        | mẽs da venda                                                        |
| 77  | YrSold:        | ano da venda                                                        |
| 78  | SaleType:      | tipo de venda                                                       |
| 79  | SaleCondition: | condição de venda                                                   |
| 80  | SalePrice:     | preço de venda                                                      |
